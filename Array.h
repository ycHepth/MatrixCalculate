...
#ifndef MATRIXCALCULATE_ARRAY_H
#define MATRIXCALCULATE_ARRAY_H
...
/* create constant vector with same constant element */
template <typename T>
...
/* create constant vector with constant array*/
template <typename T>
Vector<T>::Vector(const T* a, const unsigned int n)
        : v(new T[n])
{
...
/* resize vector with zero element and length set as input n*/
template <typename T>
...
/* resize vector with given single element and length set as input n*/
template <typename T>
...
/* change vector 1 to vector 2 with given length */
template <typename T>
...
/* obtain given indexes element of a vector */
// i.e. obtain [1,n] from vector[1,n,m], at single elements
template <typename T>
...
/* reset to <i> direction base vector */
template <typename T>
...
/* production with inner number of elements in vector */
template <typename T>
...
/* return index of max elements in vector */
template <typename T>
...
/* return index of min elements in vector */
template <typename T>
...
/* initial constant matrix with same element */
template <typename T>
Matrix<T>::Matrix(const T& a, unsigned int n, unsigned int m)
        : v(new T*[n])
{
    // const T& a means referring to constant.
    unsigned int i, j;
    this->n = n; this->m = m;
    v[0] = new T[m * n];
    for (i = 1; i < n; i++)
...
/* initial <diagonal matrix> with constant elements 'a' at diagonal, and element 'o' at rest. */
template <class T>
Matrix<T>::Matrix(MType t, const T& a, const T& o, unsigned int n, unsigned int m)
        : v(new T*[n])
{
    unsigned int i, j;
    this->n = n; this->m = m;   // dimension = nxm
    v[0] = new T[m * n];        // first row
    for (i = 1; i < n; i++)
        v[i] = v[i - 1] + m;
    switch (t)
    {
...
/* initial <diagonal matrix> with constant vector '<Vector <T>> a' at diagonal, and constant element 'o' at rest. */
template <class T>
...
/* resize matrix and clear all elements to build a empty matrix with nxm */
template <typename T>
...
/* resize matrix and set all elements as constant element 'a', to build a empty matrix with nxm */
template <typename T>
...
/* extract specified row of matrix */
template <typename T>
...
/* extract specified column of matrix */
template <typename T>
inline Vector<T> Matrix<T>::extractColumn(const unsigned int j) const
{
    ...
    for (i = 0; i < n; i++)
        tmp[i] = v[i][j];   // j is the specified column

    return tmp;
}

/* extract diagonal vector of matrix
 * when m ~= n, select minimal one of (m,n)
 */
template <typename T>
...
/* extract specified rows from index of matrix */
template <typename T>
...
/* extract specified columns from index of matrix */
template <typename T>
...
/* set specified row's element to a constant vector */
template <typename T>
...
/* set specified row to a row matrix */
template <typename T>
...
/* set specified rows to a smaller matrix
 * --- required indexes size == m.rows
 * --- required       this.m == m.cols
 */
template <typename T>
...
/* set specified column's element to a constant vector */
template <typename T>
...
/* set specified column to a constant column matrix */
template <typename T>
...
/* set specified cols to a smaller matrix
 * --- required indexes size == a.ncols
 * --- required       this.m == a.nrows
 */
template <typename T>
...
/* set matrix to submatrix with specified row/col index */
template <typename T>
...
inline bool operator==(const Matrix<T>& a, const Matrix<T>& b)
{
    if (a.nrows() != b.nrows() || a.ncols() != b.ncols())
        throw std::logic_error("Matrices of different size are not same");
...
{
    os << "size of matrix: [ " <<  m.nrows() << " , " << m.ncols() << " ]" << std::endl;
...
void svd(const Matrix<T>& A, Matrix<T>& U, Vector<T>& W, Matrix<T>& V)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        anorm = std::max(anorm, (T)fabs(W[i]) + (T)fabs(rv1[i]));
    }
